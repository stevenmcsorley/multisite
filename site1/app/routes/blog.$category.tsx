...
import { getAllBlogPostsByCategory } from "~/models/blog.server";
...
export const loader: LoaderFunction = async ({ params, request }) => {
  ...
  const limit = 21;
  const { rows, total } = await getAllBlogPostsByCategory(
    category,
    page,
    limit
  );
  return json({ posts: rows, total, page, limit, category });
};

export const meta: MetaFunction = ({ data }) => {
  const { category, total, page } = data;
  const title = `${category} - Baby Names Blog`;
  const description = `Browse ${total} blog posts about ${category}.`;
  const canonical =
    page && page > 1
      ? `https://baobaonames.com/blog/${encodeURIComponent(
          category
        )}?page=${page}`
      : `https://baobaonames.com/blog/${encodeURIComponent(category)}`;
...
export const links: LinksFunction = () => {
  return []; // Include any category-specific links if needed.
};
...
export default function BlogCategory() {
  const { posts, total, page, limit, category } = useLoaderData<{
    posts: BlogPost[];
    total: number;
    page: number;
    limit: number;
  ...
  // Reuse the same layout sections as the main blog index.
